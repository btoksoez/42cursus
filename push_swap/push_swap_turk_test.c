...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   push_swap_turk_test.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: btoksoez <btoksoez@student.42lisboa.com    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/26 22:58:40 by btoksoez          #+#    #+#             */
/*   Updated: 2024/01/15 12:19:23 by btoksoez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	push(FILE *commands, t_stack **stack_a, t_stack **stack_b, t_info *info)
{
	...
	{
		printf("\nis inbetween\n");
		while (info->cheapest_element->value > current_b->value)
			current_b = current_b->next;
		while (info->cheapest_element->value < current_b->value && current_b->next != NULL)
			current_b = current_b->next;
		if (current_b->next == NULL && info->cheapest_element->value < current_b->value)
			current_b->position += 1;
		printf("current_b pos: %d, val:%d\n", current_b->position, current_b->value);
...
void sort()
{
	while(stack_a)
	{
		find_cheapest_element(stack_a, stack_b, info);
		push(element, stack_a, stack_b);
			//while element != stack_a1
				//rotate_a
			//
		is_sorted()

		//one algorithms finding cheapest element virtually and another one pushing it in reality

		//multiple text outputs, correct rotations, choose shortest one?
		//make circuraly doubly linked list?
			//rotate = moving head only
			//adding element at right postion means traversing list until current < element and current->next > element
			//but can still push only at head.. 
	}
}
...
// 		update_info(stack_a, stack_b, info);
// 	}
// 	printf("Found cheapest Element: Val: %d, Pos: %d\n", info->cheapest_element->value, info->cheapest_element->position);
// 	push(commands, stack_a, stack_b, info); //push element with lowest cost to right position
// }

void find_cheapest_element(t_stack *stack_a, t_stack *stack_b, t_info *info)
{
	//find correct position for every element
	//calculate cost to put element in that position
		//= position in a (depending on stack_a size + position in b )
		//if both (reverse) rotate, correct cost
		//return cost
	//go through until mincost found
	//return
	...
	if (!stack_a)
		return ;
	current = stack_a;
	info->mincost = INT_MAX; //don;t need mincost in info struct; local variable enough
	while (current)
	{
		printf("Current: %d\nPush_cost: %d\n\n", current->value, push_cost(current, &stack_a, &stack_b, info));
		if (push_cost(current, &stack_a, &stack_b, info) < info->mincost)
		{
			info->cheapest_element = current;
			info->mincost = push_cost(current, &stack_a, &stack_b, info);
...
int main(int argc, char *argv[])
{
	...
	if (check_input(argv[1]))
	{
		...
		find_cheapest_element(stack_a, stack_a, info);
		print_test(file, stack_a, 'A', info);
		print_test(file, stack_b, 'B', info);
